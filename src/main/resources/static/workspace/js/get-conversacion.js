document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function loadConversaciones() {
    try {
      ...
      conversaciones.forEach((conv) => {
        ...
        li.className =
          'p-2 bg-gray-100 rounded cursor-pointer hover:bg-gray-200'
        ...
        li.addEventListener('click', async () => {
          ...
          try {
            ...
            if (!mensajesResp.ok)
              throw new Error(
                `Error al cargar mensajes: ${mensajesResp.status}`
              )
            ...
            mensajes.forEach((msg) => {
              ...
              div.textContent = msg.contentText
  ...
  //Actualiza la busqueda de conversaciones y crea una simulacion
  //De que cuando se cree una converascion aparezca en el historial
  loadConversaciones()
  setInterval(loadConversaciones, 3000)
})
